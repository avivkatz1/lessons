...
/**
 * NamingAnglesLevelFour - Complex angle naming practice
 * Students name angles in complex diagrams with multiple angles
 * Supports text input, multiple choice, and point identification interactions
 */
function NamingAnglesLevelFour({ triggerNewProblem }) {
  ...
  const handlePointClick = (pointName) => {
    ...
    const newClicked = [...clickedPoints, pointName];
    setClickedPoints(newClicked);

    // Check if we have 3 points
    if (newClicked.length === 3) {
      const userAnswer = newClicked.join('');
      // Check if correct
      if (acceptedAnswers.includes(userAnswer) || userAnswer === answer) {
        revealAnswer();
      } else {
        // Wrong answer - reset after brief delay
        setTimeout(() => {
          setClickedPoints([]);
        }, 1000);
      }
  ...
  const isPointClicked = (pointName) => clickedPoints.includes(pointName);
  const clickOrder = (pointName) => clickedPoints.indexOf(pointName) + 1;

  // Compute centroid of all points so labels push outward from center
  const centroidX = points.length ? points.reduce((s, p) => s + p.x, 0) / points.length : 300;
  const centroidY = points.length ? points.reduce((s, p) => s + p.y, 0) / points.length : 250;

  // Return label position pushed outward from the centroid
  ...
  return (
    <Wrapper>
      ...
      <QuestionSection>
        ...
        {interactionType === "identification" && clickedPoints.length > 0 && (
          <ProgressText>
            Selected so far: ∠{clickedPoints.join('')}
            {clickedPoints.length < 3 && <span> (need {3 - clickedPoints.length} more)</span>}
          </ProgressText>
        )}
      </QuestionSection>

      {/* 2. VisualSection - Complex angle diagram */}
      <VisualSection>
          ...
          <Layer>
            ...
            {indicatedAngle.vertex && (
              <Arc
                ...
                innerRadius={indicatedAngle.arcRadius - 8}
            ...
            {points.map((point, index) => {
              const isClicked = isPointClicked(point.name);
              const order = clickOrder(point.name);
              ...
              return (
                <React.Fragment key={`point-${index}`}>
                  <Circle
                    x={point.x}
                    y={point.y}
                    radius={isClicked ? 10 : 6}
                    fill={isClicked ? konvaTheme.opposite : konvaTheme.shapeStroke}
                    stroke={konvaTheme.shapeStroke}
                    strokeWidth={2}
                    onClick={interactionType === "identification" ? () => handlePointClick(point.name) : undefined}
                    onTap={interactionType === "identification" ? () => handlePointClick(point.name) : undefined}
                    style={{ cursor: interactionType === "identification" ? 'pointer' : 'default' }}
                  />
                  <Text
                    x={labelPos.x}
                    y={labelPos.y}
                    ...
                    fontSize={24}
                    fontStyle="bold"
                    fill={isClicked ? konvaTheme.opposite : "#4299E1"}
                  />
                  {isClicked && (
                    <Circle
                      x={labelPos.x + 22}
      ...
      <InteractionSection>
        {!showAnswer && (
          <>
            ...
            {interactionType === "identification" && (
              <InstructionText>
                Click the three points in order to name the angle
                (any order is fine as long as the vertex is in the middle!)
              </InstructionText>
            )}
          </>
        )}
        ...
        {showAnswer && explanation && (
          <ExplanationSection>
            <ExplanationTitle>Excellent Work!</ExplanationTitle>
            <ExplanationText>{explanation}</ExplanationText>
            <ExplanationText>
              <strong>Key Concept:</strong> In complex diagrams with many angles, always identify
              the vertex first, then trace which two rays form the angle you're naming.
            </ExplanationText>
...
export default NamingAnglesLevelFour;
