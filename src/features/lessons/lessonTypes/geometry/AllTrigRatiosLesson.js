...
const LEVEL_INFO = {
  1: { title: "Identify the Sides" },
  2: { title: "Which Ratio?" },
  3: { title: "Set Up the Equation" },
  4: { title: "Solve for the Side" },
  5: { title: "Find the Angle" },
  6: { title: "Word Problems" },
};
...
// ==================== TRIG TRIANGLE COMPONENT ====================

function TrigTriangle({ visualData, konvaTheme, canvasWidth, canvasHeight }) {
  ...
  // Label positions — midpoints of each side, offset outward
  ...
  // Determine what text to show for each side
  const sideText = (side) => {
      ...
      <Layer>
        ...
        {rightAngle && (
          <>
            {(() => {
              const sz = 12;
              // Right angle is at (x2, y2) — corner where adj meets opp
        ...
        {sideText(sides.opposite) && (
          <Text
            x={oppMidX}
            y={oppMidY - fontSize / 2}
            text={sideText(sides.opposite)}
            fontSize={fontSize}
            fontStyle="bold"
            fill={oppColor}
            listening={false}
        ...
        {sideText(sides.hypotenuse) && (
          <Text
            x={hypMidX}
            y={hypMidY - fontSize / 2}
            text={sideText(sides.hypotenuse)}
            fontSize={fontSize}
            fontStyle="bold"
            fill={hypColor}
            listening={false}
        ...
        {/* SOH-CAH-TOA reference (subtle, at bottom) */}
        <Text
          x={0}
          y={canvasHeight - 16}
          width={canvasWidth}
          align="center"
          text="SOH-CAH-TOA:  sin = opp/hyp    cos = adj/hyp    tan = opp/adj"
...
function AllTrigRatiosLesson({ triggerNewProblem }) {
  const {
    lessonProps,
    showAnswer,
    revealAnswer,
      ...
      {!isMCLevel && !showAnswer && (
        <InteractionSection>
          <AnswerInput
            ...
            disabled={showAnswer}
            placeholder={level === 5 ? "Enter angle (e.g. 45)" : "Enter answer (e.g. 7.2)"}
          />
        </InteractionSection>
      )}
...
export default AllTrigRatiosLesson;
