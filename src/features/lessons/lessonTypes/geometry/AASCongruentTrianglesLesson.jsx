/**
 * AAS (Angle-Angle-Side) Congruent Triangles Lesson
 *
 * Teaches the AAS congruence criterion:
 * Two triangles are congruent if two angles and a NON-INCLUDED side are equal.
 * The non-included side is any side that is NOT between the two known angles.
 *
...
const LEVEL_INFO = {
  1: { title: 'AAS Recognition', description: 'Identify AAS congruence' },
  2: { title: 'AAS Parts', description: 'Match AAS components' },
    ...
    <Wrapper>
      ...
      {level === 2 && phase === 'interact' && (
        <ClassificationSection>
          {showHint && hint && <HintBox>{hint}</HintBox>}
          {wrongAttempts > 0 && (
            <FeedbackText $isWrong>Not quite — check which parts prove AAS!</FeedbackText>
          )}
          <InstructionText>
            Match the corresponding parts that prove these triangles are congruent by AAS:
          </InstructionText>
...
export default AASCongruentTrianglesLesson;
