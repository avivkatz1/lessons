...
import MovablePointsAndLinesSameSideInterior from "../MovablePointsAndLinesSameSideInterior";
const SameSideInteriorButton = ({
  handleClick,
  sameSideInterior,
  x = 20,
  y = 0,
  ...
  return (
    <>
      <Circle x={x + 30} y={y + 15} fill="red" radius={4} />
      <Circle x={x + 50} y={y + 19} fill="green" radius={4} />
      ...
      <Rect
        ...
        onClick={handleClick}
      />
      {sameSideInterior == true && (
        <>
          <Text
            x={x - 600 + textPos.x}
            y={y + 40 + textPos.y}
            fontSize={20}
            text="between the parallel lines and same side of the Transversal... these angles add up to 180.  Click one to see the same-side interior pair"
            width={500}
          />
          <MovablePointsAndLinesSameSideInterior
            points={points}
...
export default SameSideInteriorButton;
